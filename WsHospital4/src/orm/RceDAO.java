...
public class RceDAO {
	...
	public static boolean deleteAndDissociate(orm.Rce rce)throws PersistentException {
		try {
			...
			if (rce.getIdHoraMedica() != null) {
				rce.getIdHoraMedica().rce.remove(rce);
			}
			...
			orm.Actividad[] lActividads = rce.actividad.toArray();
			for(int i = 0; i < lActividads.length; i++) {
				lActividads[i].rceidRce.remove(rce);
			}
			orm.Procedimiento[] lProcedimientos = rce.procedimiento.toArray();
			for(int i = 0; i < lProcedimientos.length; i++) {
				lProcedimientos[i].rceidRce.remove(rce);
			}
			orm.Diagnostico[] lDiagnosticoidDiagnosticos = rce.diagnosticoidDiagnostico.toArray();
			for(int i = 0; i < lDiagnosticoidDiagnosticos.length; i++) {
				lDiagnosticoidDiagnosticos[i].rceidRce.remove(rce);
			}
			orm.Receta[] lRecetas = rce.receta.toArray();
			for(int i = 0; i < lRecetas.length; i++) {
				lRecetas[i].setIdRce(null);
			}
			return delete(rce);
		}
		catch(Exception e) {
