...
<body>
	...
	<%
		...
		if (!paciente.equals("{\"results\":[]}")) {
			try {
				JsonElement jelement = new JsonParser().parse(paciente);
				JsonObject jobject = jelement.getAsJsonObject();
				JsonArray jarray = jobject.getAsJsonArray("results");
				jobject = jarray.get(0).getAsJsonObject();
				id = jobject.get("id").getAsString();
				fichaClinica=jobject.get("nroFicha").getAsString();
				JsonObject personaJson = jobject.get("persona").getAsJsonObject();
				apellido= personaJson.get("apellidos").getAsString();
				nombre= personaJson.get("nombre").getAsString();
				fechaNacimiento= personaJson.get("fechaNacimiento").getAsString();
				direccion= personaJson.get("direccion").getAsString();
				uuid= personaJson.get("uuid").getAsString();
				estado= personaJson.get("estado").getAsString();
			} catch (NullPointerException e) {
